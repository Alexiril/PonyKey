...
internal class Level0 : ILevel
{
    public Scene GetScene(Master master) =>
        new Scene(master, "Level0")
            ...
            .SetSound(ArchivedContent.LoadContent<SoundEffect>(master, "Level0/BackgroundMusicLevel0"))
            .SetIsLooped(true)
            .SetVolume(float.TryParse(PlayerSettings.GetValue("vl"), out var value) ? value : 1)
            .SetPlayAtStart(true)
            .AddGameObject(new("TreesGenerator"))
            .SetActive(false)
            .Transform.SetPosition(new(master.ViewportSize.X * 1.5f, master.ViewportSize.Y*.7f))
